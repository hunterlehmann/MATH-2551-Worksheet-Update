...
In an alternate universe, Atlanta is famous for her extravagant beaches and pristine waters. In this universe, it is known that Atlanta's waters are extremely wavy, and the height of the water (relative to ground level) may be modeled by the function $h(x,y)=\sin(x)\cos(y)$, for $x\in (0,2\pi)$ and $y\in (0,2\pi)$. Visualize this using a 3D graphing software, and compute the height of the highest tides as well as the depth of the lowest troughs. At which coordinates $(x,y)$ do these tides and troughs occur?
    ...
    {We are analyzing the local maxima and minima of the function $h(x,y) = \sin(x)\cos (y)$ over the domain $(0,2\pi)\times (0,2\pi)$. The critical points occur when $(\cos x\cos y ,-\sin x \sin y )=(0,0)$. From the second component, there are two cases to consider: Either $\sin x = 0$ or $\sin y = 0$. In the former case, $x = \pi$. Plugging back into the first component yields that $y = \pi/4$ or $3\pi/4$. The second case is symmetric to the first, so in total we have four critical points
\begin{equation*}
    \left(\pi, \frac{\pi}{2}\right), \left(\pi, \frac{3\pi}{2}\right), \left(\frac{\pi}{2}, \pi\right), \left( \frac{3\pi}{2},\pi\right) 
\end{equation*}
We will now look at the Hessian: 
\begin{equation*}
    H(h) = \begin{bmatrix}
        f_{xx}  & f_{xy}\\
        f_{yx} & f_{yy}
    ...
    \end{bmatrix}\implies \det H(f) = \sin^2x\cos^2y - \sin^2y \cos^2x
\end{equation*}
By a routine plug and chug, we note that the first two critical points have $\det H(h) = -1$ and the second two have $\det H(h) = 1$. So the first two critical points are saddles, whereas $\left(\frac{\pi}{2}, \pi\right)$ has $f_{xx}=1$ so it is a local min, and thus a trough. Finally, $\left( \frac{3\pi}{2},\pi\right)$ has $f_{xx}=-1$, so it is a local maximum and thus a tide. 
    }
